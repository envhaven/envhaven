...
- **Logs**: `/tmp/wa-claude.log` (mode 600; can contain message text, do not relax permissions).
- **Roles** (LIDs only; WhatsApp's privacy mode hides phone numbers from the bridge). Three slots in `<bridge-dir>/.env`, re-read on every message:
  ...
  - **`RESTRICTED_LIDS=<lid>,...`**: read-only LIDs. Claude refuses `Bash`, `Edit`, `Write`, `NotebookEdit`, and any state-mutating tool on their behalf. Only `Read`, `Glob`, `Grep`, `WebFetch`, `WebSearch`, and discussion. Enforcement is heuristic via the system prompt, not a hardened sandbox. Current: `<restricted>`.
...
- **Media**: images, videos, documents, stickers, **and voice notes** are downloaded to `<bridge-dir>/data/attachments/<jid>/<msg-id>.<ext>` and surfaced to Claude as absolute paths in a bracketed prefix (`[image attached: …]`, `[audio attached: …]`, `[document attached: …]`, etc.). The bridge does no transcription, no OCR, no preprocessing. `Read` renders images and PDFs natively; other formats handled via `Bash` on demand. Files are not auto-pruned. **Transcription** is an *adjacent service* (see below); install ad hoc when needed.
...
**Restricted role is heuristic, not a sandbox.** The system prompt instructs Claude to refuse mutation tools for restricted senders. A determined adversary could social-engineer past it. Use this role for collaborators you'd let read your code, not for hostile parties.
...
**`/tmp/wa-claude.log` is plaintext** (mode 600). Backups, snapshots, and syslog forwarding pick it up. Don't discuss secrets through the chat.
