...
- **Logs**: `/tmp/wa-claude.log` (mode 600; sender ID, role, and counts only — no message text or reply previews. Do not relax permissions: the file still carries enough metadata about timing and senders to be sensitive).
- **Roles** (LIDs only; WhatsApp's privacy mode hides phone numbers from the bridge). Three slots in `<bridge-dir>/.env`, re-read on every message:
  - **`OWNER_LID=<lid>`**: exactly one. Full trust plus authority to mutate the whitelist (`.env` keys and `data/allowed-groups.json`) via natural language. Current: `<owner-lid>`.
  - **`WHITELIST_NUMBERS=<lid>,...`**: full-trust LIDs. Same powers as owner, except cannot edit the whitelist or allowed-groups files. Current: `<whitelist>`.
  - **`RESTRICTED_LIDS=<lid>,...`**: read-only LIDs. Hard-gated at the SDK level: per-message, the bridge passes `tools: ['Read', 'Glob', 'Grep', 'WebFetch', 'WebSearch']` to the Agent SDK `query()` call, so `Bash`, `Edit`, `Write`, `NotebookEdit` are not in the model's context for those turns. The system-prompt addendum remains as defense in depth. Current: `<restricted>`.
  - `getRole()` in `src/index.ts` returns owner > full > restricted (highest match wins); the per-message system prompt addendum is built in `buildRoleAddendum()`.
- **Groups**: bot ignores group messages by default. When an **owner or full-trust** user adds the bot to a group, the bridge auto-records that group's JID in `<bridge-dir>/data/allowed-groups.json` and starts responding there. Restricted senders cannot auto-allow groups. Inside an allowed group, only senders with a role get replies; non-whitelisted members are still ignored. Edit the file to revoke a group or to manually allow one the bridge missed. Re-read on every message; no restart. Token-safe: the gate runs before any Claude call.
...
- **Conversation history**: per-WhatsApp-chat → Claude session id, stored in `<bridge-dir>/data/sessions.json`. Delete an entry to start a fresh session for that chat.
- **Daily flush**: every day at 5am local (configurable via `DAILY_RESET_HOUR` in `.env`). Per active JID: one combined Claude call updates evergreen surfaces in place per the flush prompt's decision tree (SOUL / feedback-mem / project-mem / reference-mem / TOOLS; see `src/claude.ts:buildFlushPrompt`) AND writes today's journal entry to `<bridge-dir>/data/journal/<safe-jid>/<YYYY-MM-DD>__<sessionId-8>.md` (frontmatter `date`/`session`/`tier`/`summary`, body ≤ 512 words; sparse sessions write less or skip the file). Then an aging pass walks every JID's journal: files at the correct tier for their age are **skipped (no LLM call)**, files crossing a tier boundary are re-summarized in one shot directly to the target tier, files ≥28 days old are deleted. Then SDK sessions reset globally. Catch-up on next start if down at 5am (marker: `data/last-reset.json`). Tiers (max words, not targets): T1 0–6d=512, T2 7–13d=256, T3 14–20d=128, T4 21–27d=64.
- **Cron jobs**: user-defined recurring tasks live in `<bridge-dir>/data/crons.json` (5 fields per entry: `jid`, `creatorLid`, `schedule`, `prompt`, `lastRun`). The scheduler ticks every 60s, re-reads the file, fires entries whose `nextRun(lastRun) <= now`, persists the new `lastRun` BEFORE invoking Claude (idempotency: a crash mid-fire never double-fires). Schedules are 5-field cron evaluated in `CRON_TIMEZONE` (`.env`; empty = system tz). The creator's role is re-checked at every fire; revoking them from the whitelist also stops their crons (entry stays for the operator to delete). Catch-up after downtime is **collapsed**: N missed occurrences fire as one. Mutated by Claude via natural language (no slash commands); restricted senders are blocked by the existing role gate. Source: `src/crons.ts` (scheduler), `src/index.ts:fireCron` (handler).
...
The bridge is a **trust amplifier**: a whitelisted sender's message becomes a Claude Agent SDK call running with `bypassPermissions` and full Code tools. What lands in chat as a one-line ask becomes shell access to your container by way of Claude. The threat model below follows from that.

**Whitelist = full trust by default.** Adding a non-restricted LID is "I let this person open a tmux window in my container." The owner role (`OWNER_LID`, exactly one) is bootstrapped distinct from later additions; subsequent senders are added with a role choice (full-trust or restricted) per the skill's M3 management action.

**Restricted role is SDK-gated, not just system-prompt prose.** For restricted senders the bridge passes `tools: ['Read', 'Glob', 'Grep', 'WebFetch', 'WebSearch']` to the Agent SDK `query()` call, so `Bash`, `Edit`, `Write`, `NotebookEdit` and the other mutating tools are not in the model's context for that turn. A clever message cannot prompt-inject the model into calling a tool that isn't there. The system-prompt addendum is kept as defense in depth (it shapes refusal language and covers tools added in future SDK versions). The role *assignment* itself, however, is governed by `.env`: an attacker with write access to that file can promote themselves. The boot-time `chmod 600 .env` is hygiene, not a substitute for filesystem protection. Use restricted for collaborators you'd let read your code, not for hostile parties.

**Whitelist edits require owner.** Only the LID matching `OWNER_LID` can authorize whitelist changes via natural language. Full-trust senders asking Claude to add or remove a LID, change roles, or allow or revoke a group are refused (offered a relay to the owner). Enforced in `src/index.ts:buildRoleAddendum`.
...
**`/tmp/wa-claude.log` is plaintext** (mode 600). The bridge never logs message text or reply previews — only sender ID, role, and counts — but the metadata alone (timing, sender frequency) is still sensitive. Backups, snapshots, and syslog forwarding pick it up. Don't discuss secrets through the chat: even if the log itself is clean, Claude's responses are written wherever the operator points them (a file edit lands on disk in plaintext regardless).
