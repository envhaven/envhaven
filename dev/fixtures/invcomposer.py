#!/usr/bin/env python3
"""Composer mimic that draws its own caret, for predictive-echo e2e.

composer.py, except it hides the terminal cursor (DECTCEM) and draws the caret
itself as an inverse-video cell inside the text. Applications that do this
leave the engine no real cursor to follow, and they colour the caret through an
SGR attribute rather than a background, so the predicted cursor block has to
reproduce that attribute instead of painting a block of its own over it.
Exit with ^C.
"""
import os
import re
...
def render():
    ...
    disp = list(lines)
    _ln = disp[li]
    disp[li] = (_ln[:col] + '\x1b[7m' + _ln[col] + '\x1b[27m' + _ln[col + 1:]
                if col < len(_ln) else _ln + '\x1b[7m \x1b[27m')
